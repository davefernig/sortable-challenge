...
def model_insert(family_trie, model_code, product_name):
    """
    Recursively insert a model code into the family trie
    """
    if not model_code:
    
        if 'PRODUCT_NAME' not in family_trie:
            family_trie['PRODUCT_NAME'] = product_name

    else:

        if model_code[0] not in family_trie:
            family_trie[model_code[0]] = {}
        
        model_insert(family_trie[model_code[0]], model_code[1:], product_name)
